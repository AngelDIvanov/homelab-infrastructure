...
mapfile -t ranges < <(virsh net-dumpxml "$NETWORK" \
    | sed -n "s/.*<range start='\([^']*\)' end='\([^']*\)'.*/\1 \2/p")
...
restore() {
    echo "Error: adding the new range failed; restoring the previous ranges" >&2
    for r in "${ranges[@]}"; do
        virsh net-update "$NETWORK" add ip-dhcp-range \
            "<range start='${r% *}' end='${r#* }'/>" --live --config >/dev/null || true
    done
}
for r in "${ranges[@]}"; do
    virsh net-update "$NETWORK" delete ip-dhcp-range \
        "<range start='${r% *}' end='${r#* }'/>" --live --config >/dev/null
done
