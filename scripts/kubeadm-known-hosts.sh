#!/usr/bin/env bash
# Pin the SSH host keys of the kubeadm VMs for Ansible.
#
# The keys are read through the QEMU guest agent (virsh qemu-agent-command),
# not over the network, so a host on the libvirt bridge cannot substitute its
# own key. The result is written to ansible/inventory/known_hosts, which the
# generated inventory uses with StrictHostKeyChecking=yes.
#
# Run after the VMs have booted (qemu-guest-agent is installed by cloud-init):
#   ./scripts/kubeadm-known-hosts.sh
set -euo pipefail

cd "$(dirname "${BASH_SOURCE[0]}")/.."
INVENTORY=ansible/inventory/kubeadm.ini
KNOWN_HOSTS=ansible/inventory/known_hosts
...
while read -r vm ip; do
    ...
    echo "pinned $vm ($ip)"
done < <(awk '/^kubeadm-[a-z0-9-]+ ansible_host=/ { split($2, a, "="); print $1, a[2] }' "$INVENTORY")
